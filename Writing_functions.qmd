...
Write a function which takes in two vectors `x` and `y` and returns the $R^2$ value for a linear model y \~ x. Use it to find the $R^2$ value for a linear fit between `body_mass_g` and `bill_length_mm` for the `penguins` data. Hint: use `summary` on the output of `lm`
...
scatterplot_with_lm <- function(data, x_col, y_col, grouping_var, title) {
  data %>% 
    ...
    geom_smooth(aes(color = {{grouping_var}}, fill = {{grouping_var}}), method="lm", alpha =0.1) +
...
Rewrite the code below as a function that can take any data set similar to `babynames` and plot the popularity as either the `prop` or `n` columns over year for the variable `chosen_name`. Call the function `name_popularity_plotter`.
...
Let's use a more practical example. Suppose you want to split your data set into pieces and then fit a linear model to each piece, and then extract the regression coefficient for each piece. You could use map functions to carry this out easily:
...
```{r}
# plot the proportion of babies named "John" over time
babynames %>% 
  filter(name == "John") %>% 
  ggplot(aes(x=year, y=prop)) + 
  geom_col(color="orange", fill="orange", alpha=0.5) + theme_classic()
...
Let's use `map` to find the linear rate of change of the proportion of selected baby names over time.

```{r}
selected_names <- c("John", "Tyler", "Elizabeth", "Jessica")
...
Now let's find the linear rate of change for all names with at-least 500 occurrences in the year 2000 and arrange them by highest to lowest rate:

```{r}
subset <- babynames  %>% filter(n >= 500, year == 2000)
dim(subset)

babynames %>% 
  filter(name %in% subset$name ) %>% 
  nest(.by = name) %>% 
  mutate(models = map(data, function(df) lm(prop ~ year, data=df))) %>% 
  mutate(slope = map_dbl(models, function(m) summary(m)$coefficients[2])) %>% 
  mutate(rsquared = map_dbl(models, function(m) summary(m)$r.squared)) %>% 
  filter(rsquared >= 0.1)  %>% 
  select(name, slope, rsquared) %>% 
  arrange(slope) %>% 
  View()
```
