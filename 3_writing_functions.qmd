...
Suppose I'm trying to calculate the column means of a matrix `X` . A function called `apply()` lets me run some function for each column (or row), so I can couple `apply()` with `mean()` to calculate the column means.
...
apply(X, MARGIN = 2, FUN = mean_with_na)
```

But if I'm doing this operation once, it feels wasteful having to create a new function for a single use case. In this case, I can use an anonymous function.
...
# call the function
simple_function(penguins, 1:100)
```
...
In the function we defined above, the `col_name` inside `summarize()` function is interpreted as a data-variable, and the function looks for the `col_name` column inside the data set. However, it does not exist and is not able to compute the mean for the column we want it to.

What we would like to do is have R treat `col_name` as an env-variable so we can pass the data-variable inside the env-variable. Then we can pass whatever column name to the function.
...
simple_function <- function(dataset, filter_col, mean_col, value) {
  ...
  dataset %>% 
    filter( {{ filter_col }} == value) %>% 
    summarize( "{{ mean_col }}_mean" := mean( {{ mean_col }} , na.rm=TRUE)) # assign summary column name with :=
}
