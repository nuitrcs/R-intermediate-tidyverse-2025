...
```{r}
vol_calculator <- function(x, y, z) {
  x * y * z
}

vol_calculator(1,1,1)
```
...
```{r}
age_calculator <- function(x, y=2023) {
  y - x
  }
...
```{r}
# ---- parts ------
# inputs 2 vectors x, y
# fit a lm --- lm(y ~ x)
# get the r2 from the lm -- summary(model_object)$r.squared
...
simple_function(penguins, sex, flipper_length_mm, "female")
```
...
paste_dr_function <- function(x) {
  return(paste("Dr.", x))
}

map_chr(names, paste_dr_function)
```
...
map_chr(names, function(n) paste("Dr.", n)) # use an anonymous function
map_chr(names, ~paste("Dr.", .)) # also works - shorter anonymous function
```
...
chosen_name = "Tyler"
babynames %>% 
  filter(name == chosen_name, year %in% 2000:2020) %>% 
  ggplot(aes(x=year, y=prop)) + 
  geom_col(color="orange", fill="orange", alpha=0.5) + theme_classic() +
  labs(title=chosen_name)
```
...
```{r}
map_dfr(penguins, function(d) d %>% na.omit() %>% summary())
```
...
```{r}
# extract the list of names to use for next part
data_subset <- babynames %>% filter(year==2000, n >= 5000, sex=="F")
selected_names <- data_subset$name
selected_names

#  get the rate of popularity change
babynames %>% 
  filter(name %in% selected_names, year %in% 2000:2020) %>% 
  nest(.by = name) %>% 
  mutate(models = map(data, function(df) lm(prop ~ year, data=df))) %>% 
  mutate(slope = map_dbl(models, function(m) summary(m)$coefficients[2])) %>% 
  filter(slope > 0) %>% # positive slope = increase in popularity
  select(name, slope) %>% 
  arrange(desc(slope)) # arrange in decreasing order of rate
```
...
```{r}
# extract the list of names to use for next part
data_subset <- babynames %>% filter(year==2000, n >= 500)
selected_names <- data_subset$name
head(selected_names)

# get the rate of popularity change
df <- babynames %>% 
  filter(name %in% selected_names, year >= 2000) %>% 
  nest(.by = name) %>% 
  mutate(models = map(data, function(df) lm(prop ~ year, data=df))) %>% 
  mutate(slope = map_dbl(models, function(m) summary(m)$coefficients[2])) %>% 
  mutate(r_squared = map_dbl(models, function(m) summary(m)$r.squared)) %>% # r2 calculation
  filter(r_squared >= 0.1) %>% # r2 atleast 0.1
  select(name, slope, r_squared) %>% 
...
```{r}
# greatest rise in popularity
filter(df, slope==max(slope)) # Liam

# greatest drop in popularity
filter(df, slope==min(slope)) # Nicholas

# best fit to Linear model
filter(df, r_squared==max(r_squared)) # Zackary
```
